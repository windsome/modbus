...
{
    if (ctx->debug) {
        wmprintf("ERROR errno=%s context=%s\r\n", modbus_strerror(errno), context?:"null");
    }
}
...
int _sleep_and_flush(modbus_t *ctx)
{
    ...
    remaining = request = os_msec_to_ticks(ctx->response_timeout.tv_sec * 1000 + ctx->response_timeout.tv_usec / 1000);
    //while ((remaining = os_thread_sleep(request)) > 0)
    //    request = remaining;

    os_thread_sleep(request);
...
int modbus_send_raw_request(modbus_t *ctx, uint8_t *raw_req, int raw_req_length)
{
    ...
    if (raw_req_length < 2) {
        ...
        wmprintf ("error: The raw request must contain function and slave at least\r\n");
        return -1;
    }
...
static int receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
{
    ...
    if (ctx->debug) {
        if (msg_type == MSG_INDICATION) {
            wmprintf("Waiting for a indication...\r\n");
        } else {
            wmprintf("Waiting for a confirmation...\r\n");
        }
    }
                              ...
                              uint8_t *rsp, int rsp_length)
{
    ...
    if (rsp_length == rsp_length_computed ||
        ...
        if (function != req[offset]) {
            if (ctx->debug) {
                wmprintf("Received function not corresponding to the request (%d != %d)\r\n",
                        function, req[offset]);
            }
            if (ctx->error_recovery & MODBUS_ERROR_RECOVERY_PROTOCOL) {
                _sleep_and_flush(ctx);
        ...
        if (req_nb_value == rsp_nb_value) {
            ...
            if (ctx->debug) {
                wmprintf("Quantity not corresponding to the request (%d != %d)\r\n",
        ...
        if (ctx->debug) {
            wmprintf("Message length not corresponding to the computed length (%d != %d)\r\n",
                 ...
                 int req_length, modbus_mapping_t *mb_mapping)
{
    ...
    case _FC_READ_COILS: {
        ...
        if (nb < 1 || MODBUS_MAX_READ_BITS < nb) {
            if (ctx->debug) {
                wmprintf("Illegal nb of values %d in read_bits (max %d)\r\n",
            ...
            if (ctx->debug) {
                wmprintf("Illegal data address %0X in read_bits\r\n",
                        address + nb);
            }
            rsp_length = response_exception(
                ctx, &sft,
    ...
    case _FC_READ_DISCRETE_INPUTS: {
        ...
        if (nb < 1 || MODBUS_MAX_READ_BITS < nb) {
            if (ctx->debug) {
                wmprintf("Illegal nb of values %d in read_input_bits (max %d)\r\n",
                        nb, MODBUS_MAX_READ_BITS);
            }
            rsp_length = response_exception(
                ctx, &sft,
            ...
            if (ctx->debug) {
                wmprintf("Illegal data address %0X in read_input_bits\r\n",
                        address + nb);
            }
            rsp_length = response_exception(
                ctx, &sft,
    ...
    case _FC_READ_HOLDING_REGISTERS: {
        ...
        if (nb < 1 || MODBUS_MAX_READ_REGISTERS < nb) {
            if (ctx->debug) {
                wmprintf("Illegal nb of values %d in read_holding_registers (max %d)\r\n",
            ...
            if (ctx->debug) {
                wmprintf("Illegal data address %0X in read_registers\r\n",
                        address + nb);
            }
            rsp_length = response_exception(
                ctx, &sft,
    ...
    case _FC_READ_INPUT_REGISTERS: {
        ...
        if (nb < 1 || MODBUS_MAX_READ_REGISTERS < nb) {
            if (ctx->debug) {
                wmprintf("Illegal number of values %d in read_input_registers (max %d)\r\n",
                        nb, MODBUS_MAX_READ_REGISTERS);
            }
            rsp_length = response_exception(
                ctx, &sft,
            ...
            if (ctx->debug) {
                wmprintf("Illegal data address %0X in read_input_registers\r\n",
                        address + nb);
            }
            rsp_length = response_exception(
                ctx, &sft,
    ...
    case _FC_WRITE_SINGLE_COIL:
        if (address >= mb_mapping->nb_bits) {
            if (ctx->debug) {
                wmprintf("Illegal data address %0X in write_bit\r\n",
                        address);
            }
            rsp_length = response_exception(
                ctx, &sft,
            ...
            if (data == 0xFF00 || data == 0x0) {
                ...
                if (ctx->debug) {
                    wmprintf("Illegal data value %0X in write_bit request at address %0X\r\n",
                            data, address);
                }
                rsp_length = response_exception(
                    ctx, &sft,
    ...
    case _FC_WRITE_SINGLE_REGISTER:
        if (address >= mb_mapping->nb_registers) {
            if (ctx->debug) {
                wmprintf("Illegal data address %0X in write_register\r\n",
                        address);
            }
            rsp_length = response_exception(
                ctx, &sft,
    ...
    case _FC_WRITE_MULTIPLE_COILS: {
        ...
        if (nb < 1 || MODBUS_MAX_WRITE_BITS < nb) {
            if (ctx->debug) {
                wmprintf("Illegal number of values %d in write_bits (max %d)\r\n",
            ...
            if (ctx->debug) {
                wmprintf("Illegal data address %0X in write_bits\r\n",
                        address + nb);
            }
            rsp_length = response_exception(
                ctx, &sft,
    ...
    case _FC_WRITE_MULTIPLE_REGISTERS: {
        ...
        if (nb < 1 || MODBUS_MAX_WRITE_REGISTERS < nb) {
            if (ctx->debug) {
                wmprintf("Illegal number of values %d in write_registers (max %d)\r\n",
            ...
            if (ctx->debug) {
                wmprintf("Illegal data address %0X in write_registers\r\n",
                        address + nb);
            }
            rsp_length = response_exception(
                ctx, &sft,
    ...
    case _FC_REPORT_SLAVE_ID: {
        ...
        break;
    case _FC_READ_EXCEPTION_STATUS:
        wmprintf("Error: FIXME Not implemented\r\n");
        return -1;
        break;
    ...
    case _FC_WRITE_AND_READ_REGISTERS: {
        ...
        if (nb_write < 1 || MODBUS_MAX_RW_WRITE_REGISTERS < nb_write ||
            ...
            if (ctx->debug) {
                wmprintf("Illegal nb of values (W%d, R%d) in write_and_read_registers (max W%d, R%d)\r\n",
                        nb_write, nb,
            ...
            if (ctx->debug) {
                wmprintf("Illegal data read address %0X or write address %0X write_and_read_registers\r\n",
                           ...
                           unsigned int exception_code)
{
    ...
    if (exception_code < MODBUS_EXCEPTION_MAX) {
        ...
        wmprintf ("Error: Positive exception code\r\n");
        return -1;
    }
}
                          ...
                          int addr, int nb, uint8_t *dest)
{
    ...
    if (rc > 0) {
        ...
        if (rc == -1) {
            wmprintf ("receive_msg fail!\r\n");
            return -1;
        }

        rc = check_confirmation(ctx, req, rsp, rc);
        if (rc == -1) {
            wmprintf ("check_confirmation fail!\r\n");
            return -1;
        }
...
int modbus_read_bits(modbus_t *ctx, int addr, int nb, uint8_t *dest)
{
    ...
    if (nb > MODBUS_MAX_READ_BITS) {
        if (ctx->debug) {
            wmprintf("ERROR Too many bits requested (%d > %d)\r\n",
                    nb, MODBUS_MAX_READ_BITS);
        }
        errno = EMBMDATA;
        return -1;
...
int modbus_read_input_bits(modbus_t *ctx, int addr, int nb, uint8_t *dest)
{
    ...
    if (nb > MODBUS_MAX_READ_BITS) {
        if (ctx->debug) {
            wmprintf("ERROR Too many discrete inputs requested (%d > %d)\r\n",
                    nb, MODBUS_MAX_READ_BITS);
        }
        errno = EMBMDATA;
        return -1;
                          ...
                          uint16_t *dest)
{
    ...
    if (nb > MODBUS_MAX_READ_REGISTERS) {
        if (ctx->debug) {
            wmprintf("ERROR Too many registers requested (%d > %d)\r\n",
                    nb, MODBUS_MAX_READ_REGISTERS);
        }
        errno = EMBMDATA;
        return -1;
                                ...
                                uint16_t *dest)
{
    ...
    if (nb > MODBUS_MAX_READ_REGISTERS) {
        wmprintf("ERROR Too many input registers requested (%d > %d)\r\n",
                nb, MODBUS_MAX_READ_REGISTERS);
        errno = EMBMDATA;
        return -1;
    }
...
int modbus_write_bits(modbus_t *ctx, int addr, int nb, const uint8_t *src)
{
    ...
    if (nb > MODBUS_MAX_WRITE_BITS) {
        if (ctx->debug) {
            wmprintf("ERROR Writing too many bits (%d > %d)\r\n",
                    nb, MODBUS_MAX_WRITE_BITS);
        }
        errno = EMBMDATA;
        return -1;
...
int modbus_write_registers(modbus_t *ctx, int addr, int nb, const uint16_t *src)
{
    ...
    if (nb > MODBUS_MAX_WRITE_REGISTERS) {
        if (ctx->debug) {
            wmprintf("ERROR Trying to write to too many registers (%d > %d)\r\n",
                    nb, MODBUS_MAX_WRITE_REGISTERS);
        }
        errno = EMBMDATA;
        return -1;
...

{
    ...
    if (write_nb > MODBUS_MAX_RW_WRITE_REGISTERS) {
        if (ctx->debug) {
            wmprintf("ERROR Too many registers to write (%d > %d)\r\n",
                                     ...
                                     int nb_registers, int nb_input_registers)
{
    ...
    mb_mapping = (modbus_mapping_t *)os_mem_alloc(sizeof(modbus_mapping_t));
    if (mb_mapping == NULL) {
        return NULL;
    }
    ...
    if (nb_bits == 0) {
        ...
        mb_mapping->tab_bits =
            (uint8_t *) os_mem_alloc(nb_bits * sizeof(uint8_t));
        if (mb_mapping->tab_bits == NULL) {
            os_mem_free(mb_mapping);
            return NULL;
        }
    ...
    if (nb_input_bits == 0) {
        ...
        mb_mapping->tab_input_bits =
            (uint8_t *) os_mem_alloc(nb_input_bits * sizeof(uint8_t));
        if (mb_mapping->tab_input_bits == NULL) {
            os_mem_free(mb_mapping->tab_bits);
            os_mem_free(mb_mapping);
            return NULL;
        }
    ...
    if (nb_registers == 0) {
        ...
        mb_mapping->tab_registers =
            (uint16_t *) os_mem_alloc(nb_registers * sizeof(uint16_t));
        if (mb_mapping->tab_registers == NULL) {
            os_mem_free(mb_mapping->tab_input_bits);
            os_mem_free(mb_mapping->tab_bits);
            os_mem_free(mb_mapping);
            return NULL;
        }
    ...
    if (nb_input_registers == 0) {
        ...
        mb_mapping->tab_input_registers =
            (uint16_t *) os_mem_alloc(nb_input_registers * sizeof(uint16_t));
        if (mb_mapping->tab_input_registers == NULL) {
            os_mem_free(mb_mapping->tab_registers);
            os_mem_free(mb_mapping->tab_input_bits);
            os_mem_free(mb_mapping->tab_bits);
            os_mem_free(mb_mapping);
            return NULL;
...
void modbus_mapping_free(modbus_mapping_t *mb_mapping)
{
    ...
    os_mem_free(mb_mapping->tab_input_registers);
    os_mem_free(mb_mapping->tab_registers);
    os_mem_free(mb_mapping->tab_input_bits);
    os_mem_free(mb_mapping->tab_bits);
    os_mem_free(mb_mapping);
